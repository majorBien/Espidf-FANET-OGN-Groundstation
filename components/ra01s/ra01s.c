...
void GFSKConfig(uint32_t bitrate, uint32_t fdev, uint8_t rxBw, uint16_t preambleLength, uint8_t payloadLen, bool crcOn, bool whiteningOn) {
    ...
    // Gaussian filter BT=0.5 (standard for OGN)
    modParams[3] = SX126X_GFSK_FILTER_GAUSS_0_5;
    // 24-bit frequency deviation
    ...
    // Preamble detector: 16 bits minimum before sync word
    ...
    // Packet type: variable length (first byte = length)
    if (payloadLen) {
    ...
    // Correct sync word for OGN: 0x2D, 0xF4, 0x2D, 0xF4
    uint8_t syncWord[4] = {0x2D, 0xF4, 0x2D, 0xF4};
    WriteRegister(SX126X_REG_SYNC_WORD_0, syncWord, 4);

    // Optional: set node and broadcast addresses if needed (not used in OGN)
    // WriteRegister(SX126X_REG_NODE_ADDRESS, &nodeAddr, 1);
    // WriteRegister(SX126X_REG_BROADCAST_ADDRESS, &broadcastAddr, 1);

    // Configure DIO interrupts (not used in polling mode)
